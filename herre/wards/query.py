...
class SchemaRegistry:
    ...
    async def parse_from_field(self, operation_field: Field, element, ward_key):
        ...
        if operation_field.type.kind == TypeKind.LIST:
            if operation_field.type.ofType.kind == TypeKind.OBJECT:
                model = self.wardTypeModelMapMap[ward_key][
                    operation_field.type.ofType.name
                ]
        ...
        raise SchemaException(
            f"No Object found for this TypeKind {operation_field.type}"
        )
    ...
    async def expand_from_schema(self, ward_key, query, result, collapse=False):
        ...
        for key, element in result.items():
            ...
            parsed_result[key] = await self.parse_from_field(
                operation_field, element, ward_key
            )

        if collapse:
            assert (
                len(parsed_result.items()) == 1
            ), "Cannot collapse query that has more then one operation"
    ...
    async def get_schema_for_ward(self, key: str):
        ...
        if key not in self.wardSchemaMap:
            ...
            assert isinstance(
                ward, GraphQLWard
            ), "Ward is not of instance GraphQL schemas only supported by GRAPHQL"

            introspection_query = await ward.arun(
                ParsedQuery(
                    """
                                        query Schema {
                    __schema {
...
class TypedQuery:
    ...
    def __init__(self, query: str, ward=None, ward_key=None, collapse=False) -> None:
        """A Typed Query
        ...
        assert (
            self.ward_key
        ), "Please Specifiy ward key (for schema expansion necessary, as ward can be used for multiple schemas)"
        self.ward = ward or get_ward_registry().get_ward_instance(self.ward_key)
        self.query = ParsedQuery(query)
        self.collapse = collapse

    async def arun(self, **variables):
        result = await self.ward.arun(self.query, variables=variables)
        expanded = await get_schema_registry().expand_from_schema(
            self.ward_key, self.query, result, collapse=self.collapse
        )
        return expanded
